 ...
 */
static void ks8851_wrreg16_par(struct ks8851_net *ks, unsigned int reg, unsigned int val)
{
	struct ks8851_net_par *ksp = to_ks8851_par(ks);
 ...
 * the device.
 */
static void ks8851_wrfifo_par(struct ks8851_net *ks, struct sk_buff *txp, bool irq)
{
	struct ks8851_net_par *ksp = to_ks8851_par(ks);
 ...
 */
static netdev_tx_t ks8851_start_xmit_par(struct sk_buff *skb, struct net_device *dev)
{
	...
	if (likely((ks8851_rdreg16_par(ks, KS_TXMIR) & 0x1fff) >= skb->len + 12)) {
